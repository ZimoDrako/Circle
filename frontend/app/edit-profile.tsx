  ...
  return <View style={styles.root}>
    <SafeAreaView edges={["top"]} style={styles.safe}>
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} style={styles.headerButton}><Icon name="chevron-back" size={24} color={colors.onSurface} /></Pressable>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <Pressable style={styles.banner} onPress={() => upload("banner")}>
        {banner ? <Image source={{ uri: banner }} style={styles.bannerImage} /> : <View style={styles.bannerEmpty}><Icon name="image-outline" size={28} color={colors.brandPrimary} /><Text style={styles.muted}>Add banner</Text></View>}
      ...
      <TextInput style={[styles.input, styles.bio]} value={bio} onChangeText={setBio} multiline maxLength={500} placeholder="Tell people a little about yourself…" placeholderTextColor={colors.muted} />
      <Text style={styles.count}>{bio.length}/500</Text>

      <Text style={styles.label}>Major</Text>
      <TextInput style={styles.input} value={major} onChangeText={setMajor} maxLength={120} placeholder="Your major" placeholderTextColor={colors.muted} />
      ...
      <View style={styles.wrap}>{interests.map(x => <Pressable key={x} onPress={() => removeInterest(x)} style={styles.interest}><Text style={styles.interestText}>{x}</Text><Icon name="close" size={14} color={colors.muted} /></Pressable>)}</View>
